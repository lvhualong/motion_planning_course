...
void pathFinding(const Vector3d start_pt, const Vector3d target_pt, char type);
void visRRTstarPath(vector<Vector3d> nodes , char color);
...
{     
    ...
    // 
    ROS_WARN("search with RRT*");
    pathFinding(_start_pt, target_pt, 0);
    ROS_WARN("search with LazyRRT");
    pathFinding(_start_pt, target_pt, 1); 
}
...
void pathFinding(const Vector3d start_pt, const Vector3d target_pt, char type)
{
    ...
    */ 
   //RRT* 
    ob::PlannerPtr optimizingPlanner = NULL;
    if(type == 0)
         optimizingPlanner = (std::make_shared<og::RRTstar>(si));   
    else 
         optimizingPlanner = (std::make_shared<og::LazyRRT>(si));   
    ...
    if (solved)
    {
        ...
        ROS_WARN_STREAM("find path with "<<path->getStateCount() <<" points");
