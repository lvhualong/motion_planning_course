...
count=1;
for iter = 1:3000
    x_rand=[round(rand*xL),round(rand*yL)];
    %disp(x_rand);
    plot(x_rand(1), x_rand(2), 'bo', 'MarkerSize',2, 'MarkerFaceColor','b');
    %Step 1: 在地图中随机采样一个点x_rand
    %提示：用（x_rand(1),x_rand(2)）表示环境中采样点的坐标
    
    
    [x_near, near_dist, near_index]= nearest_node(x_rand, T);
    %Step 2: 遍历树，从树中找到最近邻近点x_near 
    %提示：x_near已经在树T里

    
    
    current_dis=atan2(x_rand(1)-x_near(1),x_rand(2)-x_near(2));
    x_new = x_near + Delta.*[sin(current_dis) cos(current_dis)];
    plot(x_new(1), x_new(2), 'yo', 'MarkerSize',2, 'MarkerFaceColor','y');
    %Step 3: 扩展得到x_new节点
    %提示：注意使用扩展步长Delta
    ...
    T.v(count).dist=near_dist;          %从父节点到该节点的距离，这里可取欧氏距离
    ...
    is_reached_goal = reached_goal(x_new,[x_G, y_G], Thr);
    if(is_reached_goal==1)
        disp("find the goal");
        break;
    end
